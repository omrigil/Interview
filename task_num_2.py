...
parser = argparse.ArgumentParser(description="this script runs http server that serves files for download")
...
file_path = args.file_path  # pass the user's arguments
...
class MyHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    def do_GET(self):
        with open(file_path, 'rb') as f:
            ...
            if "no_length" not in file_name:   # check if the file name contains the sub-string "no_length"
                f_stat = os.fstat(f.fileno())
                self.send_header("Content-Length", str(f_stat.st_size))  # send the size of the file.
            self.end_headers()
            chunk = 4096   # Divide the file to chunks of 4096 and send them one by one.
            rf_chunk = f.read(chunk)  # copy the file and send it to the client.
...
def run(handler_class=MyHTTPRequestHandler, server_class=BaseHTTPServer.HTTPServer, protocol="HTTP/" + args.HTTP):
    port = args.port    # get the port that the user requested
    server_address = ('', port)
    handler_class.protocol_version = protocol
    httpd = server_class(server_address, handler_class)
    if args.HTTPS != "Not required":   # check if https is needed or not
